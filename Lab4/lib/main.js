...
var camerasModule = function () {
  ...
  return {
    findCameraByModel: function findCameraByModel(modelToFind) {
      console.log("Searching for: " + modelToFind);
      var searchedCamera = camerasArray.find(function (x) {
        return x.model === modelToFind;
      });
      console.log("Found: " + searchedCamera.model);
      return searchedCamera;
    },
    createSingleCamera: function createSingleCamera(_producer, _model, _productionYear, _usb20, _price) {
      camerasArray.push({
        producer: _producer,
        model: _model,
    ...
    printCameras: function printCameras() {
...
function Camera(producer, model, productionYear, usb20, price) {
  this.producer = producer, this.model = model, this.productionYear = productionYear, this.usb20 = usb20, this.price = price;
} // Public methods
...
function SportCamera(producer, model, productionYear, usb20, price, waterproof) {
...
var camerasClass = new Camera("Panasonic", "Lumix AB-ABCD2", 2017, true, 20000.0);
var sportCameraClass = new SportCamera("Go Pro", "Hero 5", 2019, true, 1500.0, true);
console.log("Public method, get production year: " + camerasClass.getProductionYear());
camerasModule.createSingleCamera("PanasonicAA", "LumixAA AB-ABCD2", 2017, true, 20000.0);
...
console.log("Sport camera, waterproof: " + sportCameraClass.getWaterproofProperty()); // Access to private function cause an exception
